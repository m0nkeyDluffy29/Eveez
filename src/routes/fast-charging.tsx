...
export const Route = createFileRoute("/fast-charging")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Coast-to-coast electrons. The EVeez fast-charging network engineered for the next million riders." },
    ],
  }),
  ...
  return (
    <SectionShell vertical="fast-charging">
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          <div className="text-xs uppercase tracking-[0.25em] text-muted-foreground">Live Network</div>
            ...
            <svg viewBox="0 0 800 300" className="absolute inset-0 h-full w-full">
              <defs>
                <linearGradient id="line" x1="0" x2="1">
                  <stop offset="0" stopColor="oklch(0.78 0.18 152)" stopOpacity="0" />
                  <stop offset="0.5" stopColor="oklch(0.78 0.18 152)" />
                  <stop offset="1" stopColor="oklch(0.74 0.19 55)" stopOpacity="0" />
                </linearGradient>
              </defs>
              {[60, 110, 160, 210].map((y, i) => (
                <motion.path
                  ...
                  stroke="url(#line)" strokeWidth="1.5" fill="none"
              ...
              {Array.from({ length: 12 }).map((_, i) => (
                <motion.circle
                  key={i} r="3" fill="oklch(0.95 0.05 90)"
                  initial={{ opacity: 0 }}
                  animate={{
                    cx: [0, 800], cy: [80 + (i * 17) % 160, 80 + (i * 23) % 160 + 10],
                    opacity: [0, 1, 0],
                  }}
        ...
        >
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            CCS2 + Bharat DC-001, smart load balancing, and dynamic pricing — all behind one tap of the EVeez app.
          </p>
          <ul className="mt-5 space-y-3 text-sm">
            {["Up to 120kW DC", "Smart queue & reserve", "Renewable-backed", "Pay-per-second billing"].map((x) => (
